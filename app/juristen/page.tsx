      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            ...
            {nieuweCount > 0 && (
              <span className="ml-2 text-green-700 font-medium">· {nieuweCount} nieuw</span>
            )}
            {kritiekCount > 0 && (
              <span className="ml-2 text-red-600 font-medium">· {kritiekCount} kritiek</span>
            )}
          </p>
        </div>
      </div>
              ...
              {sorted.map((intake) => (
                <tr key={intake.id} className="hover:bg-gray-50 transition">
                  <td className="px-5 py-3.5 font-medium text-gray-900">
                    ...
                    {intake.emotionele_staat === "gespannen" && (
                      <span className="ml-1.5 text-xs text-amber-600" title="Gespannen">⚠</span>
                    )}
                  </td>
                  <td className="px-4 py-3.5 text-gray-600">
                    {SITUATIE_LABELS[intake.situatie_type ?? ""] ?? intake.situatie_type ?? "—"}
                  </td>
                  ...
                  <td className="px-4 py-3.5 text-gray-600">
                    {intake.tekentermijn
                      ? new Date(intake.tekentermijn).toLocaleDateString("nl-NL", { day: "numeric", month: "short" })
                      : "—"}
                  </td>
                  ...
                  <td className="px-4 py-3.5">
                    <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                      ...
                      intake.status === "afgerond" ? "bg-gray-100 text-gray-500" :
