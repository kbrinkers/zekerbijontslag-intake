...
// âââ DATA-blok parsering ââââââââââââââââââââââââââââââââââââââââââââââââââââââ
...
function extractIntakeData(text: string): { clean: string; data: PanelData | null } {
  ...
  try {
    ...
    // Ongeldige JSON â negeer
  }
  const clean = text.replace(/\[INTAKE:[\s\S]*?\]/, "").trim();
  return { clean, data };
}

// âââ VSO-detectie âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ

const VSO_KEYWORDS = [
  "vaststellingsovereenkomst", "vso", "beÃ«indigingsovereenkomst",
...
// âââ Naam-extractie âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ

const extractNameFromMessage = (text: string): string | null => {
  const patterns = [
    /\bik\s+(?:ben|heet)\s+([A-ZÃÃÃÃÃÃÃÃÃÃÃÃÃÃÃÃÃÃÃÃÃÃÃÃÃÃÃ][a-zÃ Ã¡Ã¢Ã£Ã¤Ã¥Ã¦Ã§Ã¨Ã©ÃªÃ«Ã¬Ã­Ã®Ã¯Ã°Ã±Ã²Ã³Ã´ÃµÃ¶Ã¹ÃºÃ»Ã¼Ã]+(?:\s+[A-Z][a-z]+)*)/i,
...
// âââ Hoofd component ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
...
export default function Chat() {
    ...
    const init = async () => {
      try {
        ...
        setMessages([{
          id: crypto.randomUUID(),
          role: "assistant",
          content: "Hoi! Ik ben Lisa van ZekerbijOntslag.nl. Vertel me eens â wat speelt er bij jou?",
          timestamp: new Date(),
        }]);
      } finally {
  ...
  // âââ Panel data samenvoegen âââââââââââââââââââââââââââââââââââââââââââââââ
  ...
  // âââ State bijwerken uit gebruikersbericht ââââââââââââââââââââââââââââââââ

  const updateStateFromMessage = useCallback(
    (text: string, current: IntakeState): IntakeState => {
      const lower = text.toLowerCase();
      ...
      const salarisMatch = text.match(/(\d{2,5})\s*(?:euro{â¬|eur)?\s*(?:per maand|\/maand|bruto|,-)?/i);
      if (salarisMatch) {
  ...
  // âââ VSO-document verwerken âââââââââââââââââââââââââââââââââââââââââââââââ
  ...
  // âââ Intake opslaan âââââââââââââââââââââââââââââââââââââââââââââââââââââââ
  ...
  // âââ Bericht versturen ââââââââââââââââââââââââââââââââââââââââââââââââââââ
    ...
    async (text: string) => {
      ...
      try {
        ...
        setMessages((prev) => [...prev, {
          id: crypto.randomUUID(),
          role: "assistant",
          content: "Sorry, er ging even iets mis. Kun je dat nog een keer sturen?",
          timestamp: new Date(),
        }]);
      } finally {
        setIsLoading(false);
  ...
  // âââ Panel zichtbaarheid âââââââââââââââââââââââââââââââââââââââââââââââââ

  const showSummaryPanel =
    panelData.telefoon !== null ||
    Object.values(panelData).filter((v) => v !== null).length >= 6;
  ...
  if (!isInitialized) {
    return (
      <div className="flex-1 flex items-center justify-center">
        <div className="animate-pulse text-gray-400 text-sm">Ladenâ¦</div>
      </div>
    );
  }
    ...
    <div className="flex h-[calc(100vh-57px)]">
      ...
      <div className="flex flex-col flex-1 min-w-0">
        ...
        {(panelData.urgentie === "kritiek" || panelData.urgentie === "hoog") && (
          ...
          >
            {panelData.urgentie === "kritiek"
              ? "â  Er is spoed bij deze situatie â een jurist neemt zo snel mogelijk contact op"
              : "Er is enige tijdsdruk â een jurist neemt spoedig contact op"}
          </div>
        )}
      ...
      {/* ââ Panel toggle knop (mobiel, alleen als genoeg data) ââ */}
      {showSummaryPanel && (
        <button
        ...
        >
          {showPanel ? "Ã Sluit" : "ð Jouw gegevens"}
        </button>
      )}

      {/* ââ Data panel (rechts op desktop, overlay op mobiel) ââ */}
      {showSummaryPanel && (
        <div
          className={`
